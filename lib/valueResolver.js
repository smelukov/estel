 ...
 * @return {function}
 */
function handleFunctionToken(token) {
    if (token.fn) {
        return token.fn;
    }
...
function handleClassToken(token) {
    ...
    walker.walk(token, {
        MethodDefinition: function(methodToken, parent) {
            ...
            resolveMethod(methodToken, token.fn);
        }
    });
...
function resolveCallExpression(token) {
    ...
    if (typeof callee == 'function' && callee.token) {
        var fnScope = new Scope(callee.token.scope);
        ...
        if (callee.token.expression) {
            result = resolveToken(callee.token.body);
            result.scope = fnScope;

            if (typeof result.value == 'function') {
                result.value.token.scope = fnScope;
            }
        } else {
            walker.walk(callee.token.body, {
                ...
                ReturnStatement: function(token) {
                    result = resolveToken(token.argument);
                    result.scope = fnScope;
...
function resolveToken(token, walker) {
    ...
    switch (token.type) {
        ...
        case 'ThisExpression':
            ...
            return { resolved: !!cursor, value: cursor.thisRef };
        case 'AssignmentExpression':
