...
function resolveCallExpression(token) {
    ...
    if (typeof callee == 'function' && callee.token) {
        ...
        if (callee.token.type != 'ArrowFunctionExpression') {
            ...
            if (token.type == 'NewExpression') {
                fnScope.thisRef = newObj;
            } else if (callee.thisRef) {
                fnScope.thisRef = callee.thisRef;
            } else if (token.callee.type == 'MemberExpression') {
                fnScope.thisRef = prepareMemberExpression(token.callee).object;
            } else {
                fnScope.thisRef = undefined;
            }
        }
    ...
    if (!result) {
        if (token.type == 'NewExpression') {
            result = { resolved: true, value: newObj };
        } else {
            result = { resolved: true, value: undefined };
        }
    }
...
function resolveToken(token, walker) {
    ...
    switch (token.type) {
        ...
        case 'Identifier':
            ...
            if (!reference.value && reference.isFunctionDeclaration && reference.token) {
                return resolveToken(reference.token, walker);
            }
